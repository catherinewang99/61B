...
public class UndirectedGraph extends GraphObj {
    ...
    public Iteration<Integer> successors(int v) {
        ...
        inclusive.add(getVertices().get(0));
        for (int i = 1; i < getVertices().size(); i++) {
            inclusive.add(new ArrayList<>());
            for (int j = 0; j < getVertices().get(i).size(); j++) {
                int second = getVertices().get(i).get(j);
                inclusive.get(i).add(second);
                if (i != second) {
                    inclusive.get(second).add(i);
                }
            }
        }
        return Iteration.iteration(inclusive.get(v).iterator());
    }
