...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        // FILL THIS IN
        Iterator<Integer> segments = segment.iterator();
        int cur = segments.next();
        int tocur = segments.next();
        Road curr = _map.getLabel(cur, tocur);
        String name = curr.toString();
        double length = curr.length();
        String dir = curr.direction().fullName();
        while (segments.hasNext()) {
            cur = tocur;
            tocur = segments.next();
            Road nextR = _map.getLabel(cur, tocur);
            if (name.equals(nextR.toString())
                    && dir.equals(nextR.direction().fullName())) {
                length += nextR.length();
            } else {
                String line = String.format("%d. Take %s %s for %.1f miles",
                        seq, name, dir, length);
                System.out.println(line + ".");
                name = nextR.toString();
                dir = nextR.direction().fullName();
                length = nextR.length();
                seq++;

            }
        }
        String line = String.format("%d. Take %s %s for %.1f miles to %s.", seq,
                name, dir, length, _map.getLabel(tocur));
        System.out.println(line);
        seq++;
